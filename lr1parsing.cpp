...
set<pair<char, string>> closure(char nt, string prod)
{
    for(int i = 0; i < prod.size(); ++i)
    {
        ...
        if(c == '.')
        {
            set<pair<char, string>> ret = {make_pair(nt, prod)};
            if(i < c.size() - 1 && isTerminal(prod[i + 1]))
            {

            }
            else if(i < c.size() - 1 && isTerminal(prod[i + 1]))
            {
                for(string pro : productions[prod[i + 1]])
                {
                    mergeset(closure(prod[i + 1], "." + pro));
                }
            }
        }
    }
}
set<set<pair<char, string>>> nodes;

void transitions(set<pair<char, string>> cur)
{
    set<pair<char, string>> ret = {};
    for(char t : terminals)
    {
        for(pair<char, string> : cur)
        {
